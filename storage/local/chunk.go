...
// documented requirements for calling its methods concurrently (WRT pinning
// and locking). The doc comments spell out the requirements for each method,
// but here is an overview and general explanation:
//
// One group of methods acts on (or sets) the underlying chunk. These methods
// involve no locking, so the caller must make sure nobody else will call these
// methods concurrently, either by holding the sole reference to the chunkDesc
// (usually during loading or creation) or by locking the fingerprint of the
// series the chunkDesc belongs to. The affected methods are: add,
// maybePopulateLastTime, setChunk.
//
// Finally, there are the special cases firstTime and lastTime. lastTime
// requires to have locked the fingerprint of the series.  be pinned. That's
// because the chunkLastTime field in chunkDesc gets populated upon completion
// of the chunk (which happens while the series's fingerprint is locked). Once
// that has happened, calling lastTime does not require the chunk to be loaded
// anymore. The chunkFirstTime field in chunkDesc is populated upon creation of
// a chunkDesc, so it is alway safe to call firstTime. The firstTime method is
// arguably not needed and only there for consistency with lastTime.
...
func (cd *chunkDesc) String() string {
	hasChunk := 0
	if cd.c != nil {
		hasChunk++
	}
	return fmt.Sprintf("c%d %s %s", hasChunk, cd.chunkFirstTime, cd.chunkLastTime)
}
...
// concurrent access the caller must have locked the fingerprint of the series.
